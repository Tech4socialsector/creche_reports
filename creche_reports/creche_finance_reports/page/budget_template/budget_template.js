...
// 	var grant_id_filter = frappe.ui.form.make_control({
// 		parent: make_field('grant-id-field'),
// 		df: { label: 'Grant ID', fieldtype: 'Data', fieldname: 'grant_id' },
// 		render_input: true
// 	});
...
frappe.pages['budget-template'].on_page_load = function(wrapper) {
	...
	$(wrapper).find('.page-content').append(`
		...
		<div class="bt-wrap">
			...
			<div class="bt-filters">
				<div class="bt-filters-grid">
					<div class="bt-filter-item" id="budget-ref-name-field"></div>
					<div class="bt-filter-item" id="partner-name-field"></div>
					<div class="bt-filter-item" id="grant-id-field"></div>
					<div class="bt-filter-item" id="district-field"></div>
					<div class="bt-filter-item" id="block-field"></div>
					<div class="bt-filter-item" id="state-field"></div>
					<div class="bt-filter-item" id="no-of-creches-field"></div>
					<div class="bt-filter-item" id="financial-year-field"></div>
					<div class="bt-filter-item" id="date-of-approval-field"></div>
					<div class="bt-filter-item" id="start-date-field"></div>
					<div class="bt-filter-item" id="end-date-field"></div>
				</div>
			</div>
			...
			<div class="bt-summary-card">
				...
				<div class="bt-summary-grid">
					...
					<div class="bt-summary-item">
						<div class="bt-summary-label">District</div>
						<div class="bt-summary-value empty" id="card-district">Not set</div>
					</div>
					<div class="bt-summary-item">
						<div class="bt-summary-label">Block</div>
						<div class="bt-summary-value empty" id="card-block">Not set</div>
					</div>
					<div class="bt-summary-item">
						<div class="bt-summary-label">State</div>
						<div class="bt-summary-value empty" id="card-state">Not set</div>
	...
	// ── Partner change — only updates partner name card, never touches Grant ID ───
	function on_partner_change(partner_id) {
		var partner = all_partners.find(function(p){ return p.name === partner_id; });
		if (partner) {
			set_card('card-partner-name', partner.partner_name);
	...
	var budget_ref_name_filter = frappe.ui.form.make_control({
		parent: make_field('budget-ref-name-field'),
		df: {
			label: 'Budget Reference Name', fieldtype: 'Data', fieldname: 'budget_ref_name', reqd: 1,
			change: function(){ set_card('card-budget-ref-name', budget_ref_name_filter.get_value()); }
		},
		render_input: true
	});
	...
	var partner_name_filter = frappe.ui.form.make_control({
		parent: make_field('partner-name-field'),
		df: {
			label: 'Partner Name', fieldtype: 'Select', fieldname: 'partner_name', reqd: 1,
			options: '— Loading... —',
			change: function(){ on_partner_change(partner_name_filter.get_value()); }
		},
		render_input: true
	...
	// Grant ID — plain manual entry, never auto-filled from partner
	var grant_id_filter = frappe.ui.form.make_control({
		parent: make_field('grant-id-field'),
		df: {
			label: 'Grant ID', fieldtype: 'Data', fieldname: 'grant_id', reqd: 1,
			change: function(){ set_card_grant_pill(grant_id_filter.get_value()); }
		},
		render_input: true
	});

	var district_filter = frappe.ui.form.make_control({
		parent: make_field('district-field'),
		df: {
			label: 'District', fieldtype: 'Link', fieldname: 'district', reqd: 0,
			options: 'District',
			change: function(){ set_card('card-district', district_filter.get_value()); }
		},
		render_input: true
	});

	var block_filter = frappe.ui.form.make_control({
		parent: make_field('block-field'),
		df: {
			label: 'Block', fieldtype: 'Link', fieldname: 'block', reqd: 0,
			options: 'Block',
			change: function(){ set_card('card-block', block_filter.get_value()); }
		},
		render_input: true
	});
	...
	var state_filter = frappe.ui.form.make_control({
		parent: make_field('state-field'),
		df: {
			label: 'State', fieldtype: 'Link', fieldname: 'state', reqd: 1,
			options: 'State',
			change: function(){ set_card('card-state', state_filter.get_value()); }
		},
		render_input: true
	...
	var no_of_creches_filter = frappe.ui.form.make_control({
		parent: make_field('no-of-creches-field'),
		df: {
			label: 'No. of Crèches', fieldtype: 'Int', fieldname: 'no_of_creches', reqd: 1,
			change: function(){ set_card('card-no-of-creches', no_of_creches_filter.get_value()); }
		},
		render_input: true
	});
	...
	var financial_year_filter = frappe.ui.form.make_control({
		parent: make_field('financial-year-field'),
		df: {
			label: 'Financial Year', fieldtype: 'Link', fieldname: 'financial_year', reqd: 1,
			options: 'Financial year',
			change: function(){ set_card('card-financial-year', financial_year_filter.get_value()); }
		},
		render_input: true
	...
	var date_of_approval_filter = frappe.ui.form.make_control({
		parent: make_field('date-of-approval-field'),
		df: {
			label: 'Date of Approval', fieldtype: 'Date', fieldname: 'date_of_approval', reqd: 1,
			change: function(){ set_card('card-date-of-approval', frappe.datetime.str_to_user(date_of_approval_filter.get_value())); }
		},
		render_input: true
	});
	...
	var start_date_filter = frappe.ui.form.make_control({
		parent: make_field('start-date-field'),
		df: {
			label: 'Start Date', fieldtype: 'Date', fieldname: 'start_date', reqd: 1,
			change: function(){
				var s = start_date_filter.get_value();
				var e = end_date_filter ? end_date_filter.get_value() : '';
				if (s && e && s > e) {
	...
	var end_date_filter = frappe.ui.form.make_control({
		parent: make_field('end-date-field'),
		df: {
			label: 'End Date', fieldtype: 'Date', fieldname: 'end_date', reqd: 1,
			change: function(){
				var s = start_date_filter ? start_date_filter.get_value() : '';
				var e = end_date_filter.get_value();
				if (s && e && e < s) {
	...
	[budget_ref_name_filter, partner_name_filter, grant_id_filter,
	 district_filter, block_filter, state_filter, no_of_creches_filter, financial_year_filter,
	 date_of_approval_filter, start_date_filter, end_date_filter].forEach(function(f){ f.refresh(); });
