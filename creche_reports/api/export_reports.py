...
@frappe.whitelist(allow_guest=True)
def export_creche_utilisation_excel_one_sheet(names: Any = None):
    ...
    # Handle string / JSON / list
    if isinstance(names, str):
        try:
            names = json.loads(names)  # if JSON string
        except:
    ...
    for name in names:
        ...
        for item in (doc.get("budgets") or []):
            ...
            ws.cell(row=row, column=col_offset, value=serial_no)
            ws.cell(row=row, column=col_offset + 1, value=doc.partner_name)
            ws.cell(row=row, column=col_offset + 2, value=doc.state)
            ws.cell(row=row, column=col_offset + 3, value=doc.monthu)
            ws.cell(row=row, column=col_offset + 4, value=doc.fy)
            ws.cell(row=row, column=col_offset + 5, value=item.budget_head)
            ws.cell(row=row, column=col_offset + 6, value=item.budget_main_head)
            ws.cell(row=row, column=col_offset + 7, value=item.cost_category)
            ws.cell(row=row, column=col_offset + 8, value=item.amount)
